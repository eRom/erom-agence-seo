...
**Ping IndexNow.** Conditions : « Mise en ligne » posée, clé dans `seo/strategy.md`, un audit n0 postérieur à la mise en ligne avec un `raw/sitemap.xml` d'au moins une URL, case pas déjà cochée. Requête : `POST https://api.indexnow.org/indexnow`, corps JSON `{ host, key, keyLocation, urlList }`, `keyLocation` = `https://<host>/<clé>.txt` (toujours envoyé), `urlList` = les URL du sitemap collecté (10 000 max, très au-dessus), `Content-Type: application/json; charset=utf-8`. Coché sur 200 ou 202 avec la date et le nombre d'URL ; vide sur tout autre code, avec le code et la raison du tableau officiel : 403 = « la clé de `seo/strategy.md` n'est pas servie en `/<clé>.txt` sur la prod » (AI-02 du rapport n0 le dit aussi), 422 = « une URL n'est pas sur `host`, ou la clé n'a pas la forme attendue », 429 = « trop de soumissions, réessayer plus tard ». Corps et codes figés sur la doc officielle : `docs/recherches/2026-08-29-checklist-indexnow-bing-gsc.md`, section 3.1. Le corps de la réponse n'est pas exploité (200 « only indicates that the search engine has received your set of URLs ») ; l'accusé de réception se lit dans Bing Webmaster Tools, rapport IndexNow (consigne dans `consoles.md`).

**Sitemap chez Bing.** Conditions : ligne 5 cochée, `BING_WMT_API_KEY` dans l'environnement, « Mise en ligne » posée, case pas déjà cochée. Méthode `SubmitFeed(siteUrl, feedUrl)` de l'API JSON de Bing Webmaster Tools (`IWebmasterApi`, « Submits feed »), même base et même clé que `keywords.ts` : `POST https://ssl.bing.com/webmaster/api.svc/json/SubmitFeed?apikey=<clé>`, `Content-Type: application/json; charset=utf-8`, corps `{"siteUrl":"…","feedUrl":"…"}`, réponse `200` `{"d":null}` (exemple officiel, recherche du 29/08, section 3.2). `siteUrl` = le `Url` que `GetUserSites` rend pour ce site (c'est Bing qui le nomme, apex ou www, avec ou sans slash) ; `feedUrl` = le sitemap de prod tel que l'audit n0 l'a trouvé. Coché sur succès avec la date ; vide avec `ErrorCode` et `Message` sinon (enum `ApiErrorCode` : 3 InvalidApiKey, 4 et 5 Throttle, 11 NotFound, 13 NotAllowed, 14 NotAuthorized). 11, 13 ou 14 : droits insuffisants ou site hors du compte, consigne au client (ligne 10). 3 : « la clé de `~/.zshenv` n'est plus la bonne » (incident du 28/08). 4 ou 5 : « réessayer plus tard ». La clé n'apparaît jamais, ni dans le fichier, ni sur la sortie, ni dans un message d'erreur : tout ce qui s'écrit passe par `assertNoSecret(content, key)` de `skills/strategy/scripts/lib/keywords.ts`, et les URL de requête sont expurgées avant affichage.

**Lecture `GetUserSites`** (sans `--agir`, D26) : `GET https://ssl.bing.com/webmaster/api.svc/json/GetUserSites?apikey=<clé>`, réponse `{"d":[{"__type":"Site:#Microsoft.Bing.Webmaster.Api","AuthenticationCode":"…","DnsVerificationCode":"…","IsVerified":true,"Url":"https://…"}]}` (exemple officiel ; capturé le 29/08 avec la clé de Romain : `{"d":[]}`, aucun site dans le compte). Le site de la stratégie présent dans `d` (apex et www confondus, `sameSite`) coche la ligne 5 ; la preuve porte `IsVerified`. Son `Url` est celui que `SubmitFeed` réutilise.
...
- `check-sources.ts` sur les pages d'aide Google : `support.google.com` répond 404 à un HEAD et 200 à un GET (relevé le 29/08) ; rester en GET. Les pages d'aide `bing.com/webmasters/help/*` sont des applications JavaScript : dans `consoles.md`, elles sont nommées sans citation, hors du contrôle des sources.
...
## 12. Points instruits par la recherche pré-plan (29/08, 14 h 10 à 14 h 30)

Rapport : `docs/recherches/2026-08-29-checklist-indexnow-bing-gsc.md`. Réglés sur la doc officielle, extraits par `curl` : le corps du POST IndexNow et son tableau de codes (3.1) ; `SubmitFeed` et `GetUserSites` en JSON, exemples officiels, enum `ApiErrorCode`, rôles de délégation (3.2) ; les chemins de clics Search Console avec citations (3.3) ; 301 et 308 permanents, 302 et 307 jamais (3.4). Capturés sur chico : clé IndexNow servie en 200, sitemap de 10 URL, `GetUserSites` = `{"d":[]}` (aucun site dans le compte Bing de Romain).

Restent, portés dans le plan tels quels (rapport, section 5) :
- la réponse réelle du POST IndexNow (200 ou 202, corps ignoré), capturée à la recette AC-4 ;
- le code Bing réel pour un site hors du compte ou délégué en lecture seule (11, 13 ou 14 : traités pareil) ;
- la vérification DNS TXT avant déploiement, déduite, pas citée ;
- le format de `siteUrl` chez Bing, contourné en reprenant le `Url` rendu par `GetUserSites`.
...
## 13. Sources et échantillons vérifiés

Par la note `docs/recherches/2026-08-29-niveau-1-apis.md` (session d'idéation, sources en 200 le 29/08) : le jeton Search Console demandé en `webmasters.readonly` par posture, `sitemaps.submit` réservé au scope `webmasters` (discovery doc, révision 20260825) ; les rapports IA de Google et de Bing hors API ; `AddSiteRoles(…, isReadOnly)`.

Par le rapport `docs/recherches/2026-08-29-checklist-indexnow-bing-gsc.md` (recherche pré-plan, 21 sources en 200 le 29/08, échantillons en section 4) : les corps IndexNow, `SubmitFeed` et `GetUserSites` de la section 5.3, extraits des pages officielles par `curl` ; la clé de chico servie en prod ; `GetUserSites` réel sur le compte de Romain. Non capturés, par choix : une réponse réelle du POST IndexNow et de `SubmitFeed` (écritures), à coller dans la recette au moment d'AC-4. Les fixtures existantes : `plugin/skills/build/scripts/tests/fixtures/chico/` (manifest, pages, rapports n0 et n2, stratégie), collectées le 28/08 sur chico.
