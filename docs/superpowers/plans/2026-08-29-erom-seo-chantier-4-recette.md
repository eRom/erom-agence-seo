...
- **AC-4 : OK, avec R-3.** Romain a dit « lance --agir » à 17 h 41. Premier `--agir` : rien n'est parti, « 0 : aucune URL à soumettre : sitemap de prod vide » et `attention : ping IndexNow : 10 URL du sitemap ne sont pas sur www.commentchercherbonheur.org, non soumises`. Cause (R-3) : le sitemap de chico liste ses `<loc>` sur l'apex (`https://commentchercherbonheur.org/…`) alors que le site sert www ; le filtre F10 « même hôte que le ping » écartait tout. Correctif sur `main` : `urlsOnOrigin` (lib/actions.ts) ramène les URL sur l'origine réellement servie avec `rewriteToOrigin`, comme l'audit niveau 2 ; avertissement « 10 URL du sitemap réécrites sur https://www.commentchercherbonheur.org » ; test ajouté (260 tests). Second `--agir` : **IndexNow a répondu 202** (Accepted, validation de la clé en attente), ligne « `- [x] Ping IndexNow · action · 2026-08-29 · 202, 10 URL` » ; relance sans `--agir` : la case reste cochée, le ping n'est pas reproposé. Incertitude 1 de la recherche levée : 202, corps ignoré. « Sitemap soumis à Bing » reste « en attente » (compte Bing vide) : attendu.

## Bilan
AC-1 à AC-7 : OK. Trois correctifs pendant la recette, tous sur `main` avec leur test : R-1 (origine servie depuis `derived/pages.json` quand `raw/` est hors git), R-3 (URL du ping ramenées sur l'hôte servi). R-2 laissé (gabarit `<site>` dans la consigne J+1). Non exercé : `SubmitFeed` réel (le compte Bing de Romain est vide), le suivi réel des redirections d'un ancien site, la skill en dialogue (« c'est déployé ? ») ; le script l'a été sur tous ses chemins, et la skill en headless sur le cas « pas de stratégie ». Dans chico, non commités : `seo/checklist.md`, `seo/audits/2026-08-29-n0/` (rapport et derived).
